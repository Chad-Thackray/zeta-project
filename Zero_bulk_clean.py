...
psi0 = sym.series(psi0, p, x0 = 0.5, n = 12).removeO()
c1 = sym.series(c1, p, x0 = 0.5, n = 12).removeO()
c2 = sym.series(c2, p, x0 = 0.5, n = 12).removeO()
    
##### Estimation of remainder term
remfunc = ( (-1)**(N-1)*(t/(2*sym.pi))**(-1/4) ) * (psi0)  ## only includes C0
...
def gram(n): ## make this more accurate at some point
    """
...
def law(x, extra = 0):
    ...
    errors = 0.127*(gramlist+ extra)**(-3/4)
...
def Bulklehmans(z, step = 0.1): ## optimal step value important
    """
...
def verifyBulk(x,y):
    ...
    difficulties = law([z for z in range(x,y+1)]) ## list of gram points which fail grams law
